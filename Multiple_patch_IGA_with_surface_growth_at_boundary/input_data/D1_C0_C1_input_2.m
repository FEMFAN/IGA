function [fid,u_pre,f_pre,f_b,ttime,dt,loadcurve,D1_flag] = D1_C0_C1_input_2(n,m,p,k)

% choose boundary problem
drive_flag = 1; %=1: displacement_driven test; %=2: force_driven test
D1_flag = 1;    %=0: 2 dimensional problem; %=1: one dimensional problem
...
if drive_flag ==1
    if k ==1
        u_pre =  [1,0,2,1,0; %zero displacement in y-direction, ''makes the problem 1-D''
            ...
            4,0,2,1,0;
            5,0,2,1,0;
            1,0,1,1,0; %arbitrary displacements in x-direction for every control point, except the interface control points andd the ones close to it
            2,0,1,1,0.01;
            3,0,1,1,0.017];
    ...
    elseif k == 2
        ...
        u_pre =  [ 1,0,2,1,0;
            2,0,2,1,0; %zero displacement in y-direction, ''makes the problem 1-D''
            3,0,2,1,0;
            4,0,2,1,0;
            5,0,2,1,0;
            %arbitrary displacements in x-direction for every control point except the interface displacements and the ones close to it
            3,0,1,1,0.026;
            4,0,1,1,0.028;
            5,0,1,1,0.030];
